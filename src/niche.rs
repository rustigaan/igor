...
pub async fn process_niche<UT: UseThundercloudConfig, FS: FileSystem, IC: InvarConfig>(
    project_root: AbsolutePath,
    niches_directory: RelativePath,
    niche: &NicheName,
    use_thundercloud: UT,
    invar_config_default: IC,
    fs: FS,
    ...
    let thundercloud_directory =
        get_thundercloud_directory(&project_root, &niche, &use_thundercloud, &fs, target_dir)
            .await?;
    if let (Some(thundercloud_directory), use_fs) = thundercloud_directory {
        ...
        match use_fs {
            RealFs => {
                ...
                create_config_and_call_process_niche(
                    niche,
                    project_root,
                    use_thundercloud,
...
async fn create_config_and_call_process_niche<
    UT: UseThundercloudConfig,
    FS: FileSystem,
    TFS: FileSystem,
    IC: InvarConfig,
>(
    ...
    thundercloud::process_niche(niche, thunder_config).await?;

    Ok(())
}

async fn get_thundercloud_directory<UT: UseThundercloudConfig + Send + Sync, FS: FileSystem>(
    project_root: &AbsolutePath,
    niche: &NicheName,
    use_thundercloud: &UT,
    fs: &FS,
    target_dir: AbsolutePath,
) -> Result<(Option<AbsolutePath>, UseFileSystem)> {
    if let Some(directory) = use_thundercloud.directory() {
        ...
        substitutions.insert(
            "WORKSPACE".to_string(),
            Value::String(work_area.to_string_lossy().to_string()),
        );
        substitutions.insert(
            "PROJECT".to_string(),
...
async fn git_pull(git_remote_config: &impl GitRemoteConfig, path: &AbsolutePath) -> Result<()> {
    ...
    let mut child = Command::new("git")
        ...
        .arg("checkout")
        .arg("--detach")
        .arg(git_remote_config.revision())
        .spawn()?;
...
async fn git_clone(
    git_remote_config: &impl GitRemoteConfig,
    path: &AbsolutePath,
    dir: &str,
) -> Result<()> {
    let path_clone = PathBuf::clone(path);
    ...
    let mut child = Command::new("git")
        ...
        .arg("--branch")
        .arg(git_remote_config.revision())
        .arg(git_remote_config.fetch_url())
        .arg(dir)
        .spawn()?;
    let status = child.wait().await?;
    if status.success() {
...
mod test {
    ...
    use crate::config_model::{
        invar_config, project_config, NicheTriggers, ProjectConfig, PsychotropicConfig,
    };
    ...
    async fn test() -> Result<()> {
        ...
        let cargo_cult_toml_data = fs
            .get_content(AbsolutePath::new("CargoCult.toml", &project_root))
            .await?;
