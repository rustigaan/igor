...
pub mod test {
    ...
    pub fn test_new_thunder_config() -> Result<()> {
        ...
        // When
        let thunder_config = use_thundercloud_config.new_thunder_config(
            default_invar_config,
            fs.clone(),
            thunder_cloud_dir.clone(),
            fs.clone(),
            invar_dir.clone(),
            project_root.clone(),
        );
        ...
        // Then
        assert_eq!(
            thunder_config.use_thundercloud().directory(),
            use_thundercloud_config.directory()
        );
        assert_eq!(
            thunder_config.project_root().as_path(),
            project_root.as_path()
        );
        assert_eq!(
            thunder_config.thundercloud_directory().as_path(),
            thunder_cloud_dir.as_path()
        );
